...
namespace SB.AR.AppWeb
{
    public class PeoplePickerHelper
    {
        ...
        public static string GetPeoplePickerSearchData(SharePointContext spContext)
        {
            using (var context = spContext.CreateUserClientContextForSPHost())
            {
                return GetPeoplePickerSearchDataFromServer(context);
            }
        }
        ...
        public static string GetPeoplePickerData(SharePointContext spContext, string query, string qPrincipalType)
        {
            using (var context = spContext.CreateUserClientContextForSPHost())
            {
                //get searchstring and other variables
                var searchString = query;
                int principalType = Convert.ToInt32(qPrincipalType);
        ...
        public static string GetPeoplePickerSearchDataFromServer(ClientContext context)
        {
            try
            {
                //get searchstring and other variables
        ...
        public static FieldUserValue SPEnsureSBUser(ClientContext clientContext, string loginName)
        {
            FieldUserValue field = null;
            try
            {
                if (!string.IsNullOrEmpty(loginName))
                {
                    try
                    {
                        ...
                        if (loginName.Split('|').Length == 1)
                            user = clientContext.Web.EnsureUser(loginName.Split('|')[0]);
                        else
                            user = clientContext.Web.EnsureUser(loginName.Split('|')[1]);
         ...
         public static string GetSPUser(ClientContext clientContext, string loginName)
        {
            FieldUserValue field = null;
            try
            {
