...
namespace NSBD.Core.Mapping
{
  ...
  public class ListItemFieldMapper<TEntity> where TEntity : new()
  {
    ...
    /// <summary>
    /// Create an entity, and fill the mapped properties from the specified <see cref="ListItem"/>.
    /// </summary>
    ...
    public TEntity CreateEntity(ListItem item)
    {
    ...
    /// <summary>
    /// Fill a ListItem's properties based on the values in an entity.  
    /// </summary>
    /// <param name="ListItem"></param>
    /// <param name="entity"></param>
   
    
    public void FillListItemFromEntity(ListItem ListItem, TEntity entity)
    {
      Type entityType = typeof(TEntity);
      foreach (FieldToEntityPropertyMapping fieldmapping in this._fieldMappings)
      {
        fieldmapping.Map(entityType, entity, ref ListItem);
      }
    }

    /// <summary>
    /// Add a mapping between a field in an ListItem and a property in the entity. 
    /// </summary>
    ...
    /// <summary>
    /// Add a mapping between a field in an ListItem and a property in the entity with converter. 
    /// </summary>
    /// <param name="fieldId"></param>
    /// <param name="expression"></param>
    ...
    /// <summary>
    /// Add a mapping between a field in an ListItem and a property in the entity with bi converters. 
    /// </summary>
    /// <param name="fieldId"></param>
    /// <param name="expression"></param>
    /// <param name="convertToProperty"></param>
  ...
  public class FieldToEntityPropertyMapping
  {
    ...
    /// <summary>
    /// Map ListItem field to entity property
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="item"></param>
    /// <param name="entityType"></param>
    /// <param name="entity"></param>
    internal virtual void Map<T>(ListItem item, Type entityType, ref T entity)
    {
    ...
    /// <summary>
    /// Map entity property to ListItem field 
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <param name="entityType"></param>
    /// <param name="entity"></param>
    /// <param name="item"></param>
    internal virtual void Map<T>(Type entityType, T entity, ref ListItem item)
    {
      PropertyInfo propertyInfo = GetPropertyInfo(item, entityType);
      EnsureListFieldId(item, entityType);
    ...
    protected PropertyInfo GetPropertyInfo(ListItem item, Type entityType)
    {
      PropertyInfo propertyInfo = entityType.GetProperty(this.EntityPropertyName);
      if (propertyInfo == null)
      {
        string errorMessage = string.Format(CultureInfo.CurrentCulture, "Type '{0}' does not have a property '{1}' which was mapped to FieldID: '{2}' for ListItem '{3}'."
                                            , entityType.FullName
    ...
    protected void EnsureListFieldId(ListItem item, Type entityType)
    {
      try
      {
      ...
      catch (ArgumentException argumentException)
      {
        string errorMessage = string.Format(CultureInfo.CurrentCulture
                                            , "ListItem '{0}' does not have a field with Id '{1}' which was mapped to property: '{2}' for entity '{3}'."
                                            , item.Name
                                            , ListFieldId
                                            , EntityPropertyName
                                            , entityType.FullName);
  ...
  {
    /// <summary>
    /// The lambda what convert ListItem field to Entity property.
    /// </summary>
    internal Expression<Func<TField, TProperty>> ConvertToProperty { get; set; }

    internal override void Map<T>(ListItem item, Type entityType, ref T entity)
    {
      PropertyInfo propertyInfo = GetPropertyInfo(item, entityType);
      EnsureListFieldId(item, entityType);
  ...
  {
    /// <summary>
    /// The lambda what convert Entity property to ListItem field.
    /// </summary>
    internal Expression<Func<TProperty, TField>> ConvertToField { get; set; }

    internal override void Map<T>(Type entityType, T entity, ref ListItem item)
    {
      PropertyInfo propertyInfo = GetPropertyInfo(item, entityType);
      EnsureListFieldId(item, entityType);
  ...
  /// <summary>
  /// Exception that can occur when mapping a field of an ListItem to a property of a business entity. 
  /// </summary>
  [Serializable]
